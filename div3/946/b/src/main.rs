...
fn solve(s: &mut String) {
    ...
    for (i,c) in char_list.iter().enumerate() {
        map.insert(*c, char_list[l - i - 1]);
    }
    let result:Vec<char> = chars.iter().map(|c| *map.get(c).unwrap()).collect();
...
use std::{
    collections::{HashMap, HashSet}, fmt::Debug, io::stdin, ops::{Add, AddAssign, Div, Mul, MulAssign, Rem, RemAssign, Sub}, str::FromStr
};
