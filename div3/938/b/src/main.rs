...
fn solve(s: &mut String) {
    let [n, c, d] = read_vals_t(s, 0u64);
    let mut arr: Vec<u64> = read_arr(s);
    arr.sort();
    let mut v = vec![0; n as usize];
    let mut map = HashMap::new();
    let a11 = arr[0];
    for i in 0..(n as usize) {
        map.insert(a11 + ((i as u64) * d), vec![i]);
    }
    for elt in arr {
        let (pos, remove) = if let Some(poses) = map.get_mut(&elt) {
            let mut pos = poses.pop().unwrap();
            while v[pos] >= n {
                if poses.is_empty() {
                    println!("no");
                    return;
                }
                pos = poses.pop().unwrap();
            }
            (pos, poses.is_empty())
        } else {
            println!("NO");
        ...
        v[pos] += 1;
        if let Some(poses) = map.get_mut(&(elt + c)) {
            poses.push(pos);
        } else {
            map.insert(elt + c, vec![pos]);
        }
    }
    //println!("v: {v:?}");
    for v in v {
