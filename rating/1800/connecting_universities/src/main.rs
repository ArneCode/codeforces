...
fn dfs(verts: &Vec<Vec<usize>>, u_set: HashSet<usize>, depth: u64)
fn solve(s: &mut String) {
    let [n, k] = read_vals_t(s, 0u64);
    let us: Vec<usize> = read_arr(s);
    let mut u_set = HashSet::with_capacity(us.len());
    for u in us {
        u_set.insert(u - 1);
    }
...
use std::{
    collections::HashSet, fmt::Debug, io::stdin, ops::{Add, Div, Sub}, str::FromStr, vec
};
