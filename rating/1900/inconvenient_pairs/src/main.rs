...
fn solve(s: &mut String) {
    let [n, m, k] = read_vals_t(s, 0usize);

    let xs: Vec<u32> = read_arr(s);
    let ys: Vec<u32> = read_arr(s);

    let mut amnt_x = vec![vec![]; xs.len()];
    let mut amnt_y = vec![vec![]; ys.len()];

    for _ in 0..k {
        let [x, y] = read_vals_t(s, 0u32);

        let ix = binary_edge(|i| x <= xs[i], 0, xs.len());
        let iy = binary_edge(|i| y <= ys[i], 0, ys.len());
    ...
    for a in amnt_x {
        ...
        for v in a {
            if let Some(amnt) = map.get_mut(&v) {
                *amnt += 1;
            }else{
                map.insert(v, 1);
            }
