...
fn solve(s: &mut String) {
    ...
    for c in &chars {
        ...
        r += (new_x_end - new_x_start + 1) * (new_y_end - new_y_start + 1);

        if new_x_start <= x_end && new_y_start <= y_end {
            r -= (new_x_end.min(x_end) - new_x_start + 1) * (new_y_end.min(y_end) - new_y_start + 1);
        }
        ...
        // println!("y: ({y_start}, {y_end}), x: ({x_start}, {x_end})");
    }
