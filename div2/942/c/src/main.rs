...
fn solve(s: &mut String) {
    ...
    if max_i == arr.len() - 1 {
        highest += (k - cost) / n;
        let r = (highest - 1) * (n) + 1 + (k - cost) % n;
        result!("{r}");
    } else {
        let left = arr.len() - 1 - max_i;
        let left = left as u64;
        highest += (k - cost) / (max_i as u64 + 1);
        let r = (highest - 1) * n + 1 + (k - cost) % (max_i as u64 + 1) + left;
        result!("{r}");
    }
}
fn main() {
