...
fn depth(verts: &Vec<Vec<usize>>, node: usize, depths: &mut Vec<u64>) {
    for child in &verts[node] {
        if depths[*child] == 0 {
            depth(verts, *child, depths);
        }
    }
    depths[node] = verts[node].iter().map(|c| depths[*c]).max().unwrap() + 1;
