...
fn solve(s: &mut String) {
    let [n_c] = read_vals_t(s, 0usize);

    let mut circs = vec![];

    for _ in 0..n_c {
        let [x, y] = read_vals_t(s, 0i64);
        circs.push((x, y));
    }

    let [xs, ys, xt, yt] = read_vals_t(s, 0i64);

    let mut distsr = (xs - xt) * (xs - xt) + (ys - yt) * (ys - yt);


    for (x, y) in circs {
        let dist = (x - xt)*(x - xt) + (y - yt)*(y - yt);
