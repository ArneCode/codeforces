...
fn solve(s: &mut String) {
    ...
    let mut arr = vec![vec![0; chars.len()]; chars.len()];

    let mut v = 0;
    for j in 1..chars.len() {
        if chars[0] == chars[j] || chars[0] == '?' || chars[j] == '?' {
    ...
    for i in 1..chars.len() {
        ...
        for j in (i + 1)..chars.len() {
            v = v.max(arr[i - 1][j - 1]).min(j - i);
            if chars[i] == chars[j] || chars[i] == '?' || chars[j] == '?' {
                v = v.max(arr[i - 1][j - 1] + 1).min(j - i);
            }
            arr[i][j] = v;
        }
        max = max.max(v);
