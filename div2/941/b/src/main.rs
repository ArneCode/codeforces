    ...
    for y in 0..n {
        for x in 0..m {
            if data[y][x] {
                ...
                right = Some(if let Some(right) = right {
                    right.max(x)
                } else {
                    x
                });
                bottom = Some(if let Some(bottom) = bottom {
                    bottom.min(y)
                } else {
                    y
                });
                top = Some(if let Some(top) = top { top.max(y) } else { y });
            }
        }
    }

    if left.is_some() && right.is_some() && top.is_some() && bottom.is_some() {
        if left.unwrap() == 0 && right.unwrap() == m - 1 && top.unwrap() == n - 1 && bottom.unwrap() == 0 {
            println!("yEs");
            return;
        } 
    }
