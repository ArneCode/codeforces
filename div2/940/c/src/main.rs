...
fn solve(s: &mut String, dp: &Vec<u64>) {
    // n ways to place 1
    // n*n - n ways to place 2
    // let mut dp = vec![1];
    // for n in 1..1000 {
    //     if n > 1 {
    //         dp[n] += (dp[n - 2]) * (n * n - n);
    ...
    //     dp[n] += dp[n - 1] * n;
    // }
    let [ mut n, k] = read_vals_t(s, 0usize);
    for _ in 0..k {
        let [r, c] = read_vals_t(s, 0u64);
        if r == c {
            n -= 1;
        }else{
            n -= 2;
...
fn main() {
    ...
    let MOD: u64 = 1000_000_000 + 7;

    let mut dp = vec![1];
    for n in 1..400_000 {
        dp.push(0);
        if n > 1 {
            dp[n] += ((dp[n - 2]) * (((2*n as u64) - 2) % MOD))%MOD;
        }
        dp[n] += (dp[n - 1]) % MOD;
        dp[n] %= MOD;
    }
